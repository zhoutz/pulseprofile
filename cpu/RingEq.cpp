...
main() {
  ...
  std::vector<double> output_phase_grid = linspace(0, 0.96875, 32);
  ...
  Interstellar interstellar("model_data/interstellar_phot_frac.txt");

  NSX<nsx_H_v200804> nsx;

  LensingTable lt;

  auto& output_E_grid = instrument.Eo;
  ...
  std::vector<double> cos_sigma_primes(N_phase + 1);
  ...
  for (auto const& ring : grid) {
    ...
    constexpr double c0e = -0.791, c1e = 0.776, c0p = 1.138, c1p = -1.431, d1e = -1.315,
                     d2e = 2.431, f1e = -1.172, d1p = 0.653, d2p = -2.864, f1p = 0.975,
                     d160 = 13.47, d260 = -27.13, f060 = 1.69;
    double ce = c0e + c1e * x;
    double cp = c0p + c1p * x;
    ...
    for (int i_phase = 0; i_phase < N_phase; ++i_phase) {
      ...
      do {
        ...
        // fluxes_over_I[i_phase] = uu * delta3 * cos_sigma * lf * (dS) / D2;
    ...
    cos_sigma_primes[N_phase] = cos_sigma_primes[0];
    ...
    for (int i_output_phase = 0; i_output_phase < output_phase_grid.size(); ++i_output_phase) {
      for (auto const& patch : ring.patches) {
        ...
        for (int i_output_E = 0; i_output_E < output_E_grid.size(); ++i_output_E) {
          double E_obs_in_keV = output_E_grid[i_output_E];
          double E_emit_in_keV = E_obs_in_keV * redshift_factor;

          double logEkT = std::log10(E_emit_in_keV / kT_in_keV);
          if (mu < nsx.mu_vec.back()) {
            mu = nsx.mu_vec.back();
          }
          double logIT3 = nsx.Interp_logIT3(logT, logg, logEkT, mu);
          double I = logIT3 * std::pow(10., 3. * logT) / 6.62607015E-27;

          // double I = Ibb(E_emit_in_keV, kT_in_keV);
  ...
  for (int i_output_E = 0; i_output_E < output_E_grid.size(); ++i_output_E) {
    double E_obs_in_keV = output_E_grid[i_output_E];
    double fac = interstellar(E_obs_in_keV);
    for (int i_output_phase = 0; i_output_phase < output_phase_grid.size(); ++i_output_phase) {
      total_flux(i_output_phase, i_output_E) *= fac / E_obs_in_keV;
    }
  }

  Matrix total_counts(instrument.n_ch, output_phase_grid.size());
  std::ofstream total_counts_file("total_counts.txt");

  for (int i_ch = 0; i_ch < instrument.n_ch; ++i_ch) {
    for (int i_phase = 0; i_phase < output_phase_grid.size(); ++i_phase) {
      double sum = 0;
      for (int i_E = 0; i_E < output_E_grid.size(); ++i_E) {
        sum += total_flux(i_phase, i_E) * instrument.RSP(i_ch, i_E);
      }
      total_counts(i_ch, i_phase) = sum;
      total_counts_file << i_ch << " " << i_phase << " " << sum << "\n";
    }
  }
  total_counts_file.close();
  printf("Total counts written to ./total_counts.txt\n");
}
